...
const CoursePricing = ({ year, showYear10Note = false }) => {
  const pricingData = {
    "Year 7": {
      price: 61,
      subjects: ["Math"],
  ...
  // For years 7-10 maths, use $61 as the displayed price
  const displayPrice = showYear10Note ? 61 : pricingData[year]?.price || 61;
  const currentData = pricingData[year] || pricingData["Year 7"];
              ...
              {showYear10Note && (
                ...
                >
                  <p className="text-yellow-900 text-xs md:text-sm">
                    <span className="font-semibold">Note:</span> Year 10 is $93
                    per lesson, but we use $61 as the overall price for Year
                    7-10 Maths courses.
                  </p>
                </motion.div>
              )}
